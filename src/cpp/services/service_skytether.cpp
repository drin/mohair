...
namespace skytether::services {
  ...
  int SetDefaultLocation(Location* srv_loc) {
    ...
    if (not result_tcploc.ok()) {
      skytether::PrintError("Error creating default location", result_tcploc.status());
...
namespace skytether::services {
  ...
  EngineService::MakeFlightInfo(fs::path arrow_fpath, bool is_feather) {
    ...
    if (is_feather) {
      ARROW_ASSIGN_OR_RAISE(data_table, skytether::ReadIPCFile(arrow_fpath));
    }

    else {
      ARROW_ASSIGN_OR_RAISE(data_table, skytether::ReadIPCStream(arrow_fpath));
    }
  ...
  Status EngineService::DoViewChange( [[maybe_unused]] const ServerCallContext&  context
                                     ...
                                     ,[[maybe_unused]] unique_ptr<ResultStream>* result) {
    SkytetherDebugMsg("Receiving view change");
